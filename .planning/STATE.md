...
Last activity: 2026-05-15 -- Completed quick task 260515-kw1: Replace 8 /stats chart views with richer visualizations (lollipop, sankey, dual-axis cumulative+monthly, streamgraph, punchcard bubble matrix, gantt timeline strip, radial gauge, sparkline ring buffer, big-number HTML tile). 5 atomic commits on branch `feat/stats-chart-overhaul` (final HEAD `ea9f85c2`). New helpers commitPunchcard + monthlyForks + startOfUtcMonth in github-stats.service.ts. New regression test tests/stats-chart-overhaul.test.js (16/16 assertions PASS) wired into npm test chain. Zero new npm deps -- sankey implemented as inline SVG, radial gauge via Chart.js half-doughnut. All view IDs preserved (deep-link compatible). SSR contract preserved (chart.js + plugins stay inside afterNextRender).
