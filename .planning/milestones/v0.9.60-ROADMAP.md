...
- [x] **Phase 244: Hardening, Regression, MCP 0.8.0 Release** -- All v0.9.36 + dashboard + bridge contract tests pass unchanged; new multi-agent regression suite; SW-eviction reconciliation tests; SDK bump `^1.27.1` -> `^1.29.x`; `fsb-mcp-server@0.8.0` prepared for tag-driven publish with CHANGELOG + README (completed 2026-05-06)
...
**Plans**: 3 plans
  - [x] 240-01-PLAN.md -- Registry extension: token-aware bindTab/isOwnedBy, getTabMetadata sync accessor, getOrRegisterLegacyAgent carve-out, getAgentWindowId per-agent windowId pin (OWN-04 partial; D-02, D-04, Open Q2)
...
**Success Criteria** (what must be TRUE):
  ...
  5. `fsb-mcp-server@0.8.0` is prepared for npm publication via the existing tag-driven publish workflow; the final publish action remains user-gated.
**Plans**: 3 plans
...
| MCP-01, 02, 07, 08 | 244 | SDK bump, tool descriptions, tag-ready publish package |
...
| 239 -- MCP run_task Return-on-Completion | 3/3 | Complete | 2026-05-06 |
| 240 -- Tab-Ownership Enforcement | 3/3 | Complete    | 2026-05-06 |
...
| 246 -- Agent-Scoped Tab Resolution | 3/3 | Complete    | 2026-05-06 |
| 247 -- Recovery Tools Bootstrap From Restricted Active Tab | 1/1 | Complete | 2026-05-08 |

### Phase 246: agent-scoped-tab-resolution-background-default

**Goal:** Replace chrome.tabs.query({active:true}) with agent-registry-driven tab resolution across read tools, visual-session, and action tools; default open_tab to background; ensure MCP-routed surfaces never steal focus from the user or other agents. Closes the multi-agent concurrency gap surfaced in the v0.9.60 smoke test (read tools and visual-session attached to the user's foreground tab regardless of which agent owned it). Introduces resolveAgentTabOrError(agentId, params, client) helper consumed by 3 call families (read / visual / action); single legacy:* branch preserves popup/sidepanel/autopilot UX byte-for-byte; resolved tabId folded back into routeParams so Phase 240's gate enforces (agentId, tabId, ownership_token) on every non-creating tool call.
**Requirements**: Gap-closure on existing OWN-01..05 + AGENT-04 (no new REQ-IDs); design contracts D-01..D-16 from CONTEXT.md
**Depends on:** Phase 245
**Plans:** 3/3 plans complete

Plans:
- [x] 246-01-PLAN.md -- Resolver helper + 6 read tool migrations + open_tab background default + read-only.ts overturn (D-01..D-08, D-12)
- [x] 246-02-PLAN.md -- Visual session + ~37 action tool resolver migration + D-16 routeParams.tabId injection + vault overturn + 35 action-tool tab_id schemas + 5 PARAM_TRANSFORMS forwards (D-09..D-11, D-13..D-16)
- [x] 246-03-PLAN.md -- tool-definitions byte-identity test + 4 multi-agent regression scenarios (A/B/C/D) + read-only.ts agentId threading test + MCP description hygiene + full phase gate

### Phase 247: recovery-tools-bootstrap-from-restricted-active-tab

**Goal:** Fix the protocol bug where a registered MCP agent with zero owned tabs cannot recover when the active tab is a restricted browser-internal page (`chrome://newtab/`). Bootstrap-safe recovery tools (`open_tab`, `navigate`, `switch_tab`, `list_tabs`) must work without content-script attachability while still rejecting cross-agent owned tabs. MCP error mapping must label ownership/resolver/protocol failures accurately instead of falling through to generic `Page navigation`.
**Requirements**: Gap-closure on existing OWN-01..05 + AGENT-04 (no new REQ-IDs); design contracts in `247-CONTEXT.md`
**Depends on:** Phase 246
**Plans:** 1/1 plans complete

Plans:
- [x] 247-01-PLAN.md -- Bootstrap-safe tool classification, claim-aware navigate/switch_tab, open_tab zero-owned MCP path, restricted recovery truthfulness, protocol error mapping
