# Requirements Archive: v0.9.35 MCP Plug-and-Play Reliability
...
All 20 v1 requirements were marked complete at close based on the shipped Phase 198-202 summaries, verification artifacts, and dated UAT evidence.

This milestone was archived without a standalone `v0.9.35-MILESTONE-AUDIT.md` file. Phase 202 also preserves explicit residual live-UAT risk: paid-model prompt sessions through Claude Code, Codex, and OpenCode were not auto-triggered when local host preflight was already red or unconfigured and those runs can spend real provider credits.
...
- [x] **PLAT-01**: `install --all` and per-platform install commands verify idempotent config writes for Claude Desktop, Claude Code, Codex, Cursor, Windsurf, and currently supported config-file hosts.
- [x] **PLAT-02**: Codex TOML install preserves existing user config and writes the expected `mcp_servers.fsb` entry in the active Codex config shape.
- [x] **PLAT-03**: Claude/OpenClaw/OpenCode-style hosts are either supported with verified install snippets/config writes or explicitly diagnosed as unsupported with a manual fallback.
- [x] **PLAT-04**: Installer output tells the user exactly which host must be restarted/reloaded and which changes are already active.

### Validation

- [x] **VALID-01**: Automated bridge lifecycle smoke coverage proves server-before-extension, extension-before-server, server restart, and service-worker wake recovery scenarios.
- [x] **VALID-02**: Automated MCP tool smoke coverage proves `list_tabs`, `navigate`, `read_page`, `get_dom_snapshot`, `click`, `run_task`, `stop_task`, and `get_logs`.
- [x] **VALID-03**: Manual cross-host UAT captures evidence for Claude, Codex, and OpenClaw/OpenCode-style host behavior using the same smoke checklist.
- [x] **VALID-04**: Release docs include a short "when something fails" flow that points users to doctor/status instead of manual tinkering.
...
| PLAT-01 | Phase 201 | Complete |
| PLAT-02 | Phase 201 | Complete |
| PLAT-03 | Phase 201 | Complete |
| PLAT-04 | Phase 201 | Complete |
| VALID-01 | Phase 202 | Complete |
| VALID-02 | Phase 202 | Complete |
| VALID-03 | Phase 202 | Complete |
| VALID-04 | Phase 202 | Complete |
...
*Last updated: 2026-04-24 after Phase 202 completion*
