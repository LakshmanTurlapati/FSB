...
- [x] **LIVE-03**: Step descriptions update in real-time as each action executes (not just per-iteration)
...
- [x] **PROG-04**: Multi-site and Sheets workflows show task-specific progress (e.g., "Company 3/10", "Row 15/30") in the progress overlay with phase-aware ETA
...
- [x] **UX-01**: Progress overlay shows a brief task summary line (generated by AI) that gives the user a one-line understanding of the current automation goal
- [x] **UX-02**: When actions fail and recovery is attempted, overlay shows "Recovering..." or similar rather than raw error text
- [x] **UX-03**: Overlay transitions smoothly between phases (analyzing → thinking → acting) with appropriate labels and no flicker
