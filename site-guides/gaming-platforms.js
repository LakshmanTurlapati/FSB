/**
 * Site Guide: Gaming Platforms
 * Covers Steam, Epic Games Store, GOG, Humble Bundle
 */

const GAMING_PLATFORMS_GUIDE = {
...
SIGN IN:
- Sign in link: .global_action_link with text "sign in" or use XPath //a[normalize-space(.)="sign in"]
- Login page: store.steampowered.com/login -- standard form with username and password fields
...
GAME LISTINGS:
- Featured capsules contain game title, price, and tags (Top Seller, Free To Play, Early Access)
- Search results show game cards with title, price, review score, and tags
- On a game's store page, the price area shows current price, any discount, and original price
...
- Game cards show title, price, and discount info
- Epic often has exclusive deals and free weekly games

GOG (gog.com):
- DRM-free game store
- Search: use the search input in the top nav
- Game cards show title, price, discount, and GOG rating
...
- Charity component shown on bundle pages
...
5. Extract the price from the game's store page
6. Report the price, any discounts, and availability`,
  ...
  workflows: {
    ...
    browseStore: [
      'Navigate to the store section or category',
...
registerSiteGuide(GAMING_PLATFORMS_GUIDE);
