...
const SiteGraph = (function () {
  ...
  function transformTaskData(taskMemory) {
    ...
    // Collect URLs from timeline url field (preferred) or targets that look like URLs
    for (var i = 0; i < timeline.length; i++) {
      var entryUrl = timeline[i].url || '';
      if (entryUrl && entryUrl.match(/^https?:\/\//)) {
        urlsInOrder.push(entryUrl);
      } else {
