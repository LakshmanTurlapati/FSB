...
const SiteGraph = (function () {
  ...
  function transformTaskData(taskMemory) {
    ...
    var domain = sess.domain || taskMemory.metadata?.domain || 'site';
    ...
    // Track pages and elements from timeline
    var pageNodes = {}; // url -> nodeId
    var prevPageId = null;
    ...
    for (var i = 0; i < timeline.length; i++) {
      ...
      // Add page node if new
      var pageId = 'page:' + pagePath;
      if (!nodeIdSet[pageId] && url) {
        nodes.push({
        ...
        // Link page to domain root
        links.push({ source: rootId, target: pageId, type: 'nav' });
      }

      // Navigation link between consecutive pages
      var currentPageId = nodeIdSet[pageId] ? pageId : null;
      if (currentPageId && prevPageId && currentPageId !== prevPageId) {
        // Check if link already exists
        var linkExists = false;
        for (var li = 0; li < links.length; li++) {
          if (links[li].source === prevPageId && links[li].target === currentPageId && links[li].type === 'nav') {
            linkExists = true;
            break;
          }
        }
        if (!linkExists) {
          links.push({ source: prevPageId, target: currentPageId, type: 'workflow' });
        }
      }
      if (currentPageId) prevPageId = currentPageId;

      // Add element node for interaction target
      if (entry.target && currentPageId) {
        var elId = 'el:' + i + ':' + entry.target;
        if (!nodeIdSet[elId]) {
          var elLabel = (entry.action || '') + ' ' + (entry.target || '');
          if (elLabel.length > 30) elLabel = elLabel.substring(0, 29) + '\u2026';
          nodes.push({
            id: elId, type: 'element', label: elLabel,
            selector: entry.target, elType: 'el',
            parentPage: pagePath, depth: 2, size: 5
          });
          nodeIdSet[elId] = true;
          links.push({ source: currentPageId || rootId, target: elId, type: 'element' });
        }
      }
    }

    // Add discovered pages from learned.siteStructure
    if (learned.siteStructure && learned.siteStructure.pages) {
      var discoveredPages = learned.siteStructure.pages;
      var dpKeys = Object.keys(discoveredPages);
      for (var d = 0; d < dpKeys.length; d++) {
        var dpPath = dpKeys[d];
        var dpId = 'page:' + dpPath;
        if (!nodeIdSet[dpId]) {
          var dpTitle = discoveredPages[dpPath].title || dpPath;
          nodes.push({
