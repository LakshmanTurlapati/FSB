...
class MemoryStorage {
  ...
  async getIndex() {
    ...
    try {
      ...
      const index = result[MEMORY_INDEX_KEY] || { domain: {}, taskType: {}, tags: {} };
      this._indexCache = index;
      return { ...index };
    } catch (error) {
      console.error('[MemoryStorage] Failed to load index:', error);
      return { domain: {}, taskType: {}, tags: {} };
    }
  }
  ...
  async deleteAll() {
    try {
      ...
      await chrome.storage.local.set({
        [MEMORY_STORAGE_KEY]: [],
        [MEMORY_INDEX_KEY]: { domain: {}, taskType: {}, tags: {} }
      });
      return true;
    } catch (error) {
  ...
  async query(filters = {}) {
    ...
    if (!filters.domain && !filters.taskType && !filters.tags?.length && !filters.type) {
      return memories;
    }
