...
export interface ConfigResult {
  status: 'created' | 'updated' | 'skipped' | 'removed' | 'not-found' | 'error';
  path: string;
  message: string;
}
 ...
 * @returns Parsed data
 */
function parseByFormat(raw: string, format: ConfigFormat): Record<string, unknown> {
  switch (format) {
    case 'json':
 ...
 */
export function serializeByFormat(data: Record<string, unknown>, format: ConfigFormat): string {
  switch (format) {
    case 'json':
    case 'jsonc':
...
function mergeServerEntry(
  ...
  if (mergeStrategy === 'named-array') {
    ...
    const idx: number = arr.findIndex((item: Record<string, unknown>) => item.name === serverName);
    if (idx >= 0) {
    ...
    (data[serverMapKey] as Record<string, unknown>)[serverName] = entry;
  }
}
...
function checkIdempotent(
  ...
  if (mergeStrategy === 'named-array') {
    ...
    const map = data[serverMapKey] as Record<string, unknown> | undefined;
    if (!map?.[serverName]) return false;
 ...
 */
export async function installToConfig(
  configPath: string,
  ...
  try {
    ...
    const dir: string = dirname(configPath);
    if (!existsSync(dir)) {
      return {
        status: 'error',
        path: configPath,
    ...
    const fileExists: boolean = existsSync(configPath);
    if (fileExists) {
    ...
    return {
      status: 'error',
      path: configPath,
      message: 'Failed to configure ' + platform.displayName + ': ' + (err as Error).message,
    };
  }
}
...
export async function removeFromConfig(
  ...
  try {
    ...
    if (strategy === 'named-array') {
      ...
      const arr = data[platform.serverMapKey!] as Array<Record<string, unknown>>;
      const idx: number = arr.findIndex((item: Record<string, unknown>) => item.name === serverName);
      if (idx < 0) {
        return {
          status: 'not-found',
      ...
      // Object map format
      const map = data[platform.serverMapKey!] as Record<string, unknown> | undefined;
      if (!map?.[serverName]) {
        return {
          status: 'not-found',
          path: configPath,
    ...
    return {
      status: 'error',
      path: configPath,
      message: 'Failed to remove from ' + platform.displayName + ': ' + (err as Error).message,
    };
  }
}
