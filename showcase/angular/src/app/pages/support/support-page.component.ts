...
const SUPPORT_FAQ: ReadonlyArray<{ q: string; a: string }> = [
  {
    q: 'How do I install FSB?',
    a: 'Install FSB from the Chrome Web Store for normal use so Chrome applies release updates automatically. Use Load unpacked only for local development or when you need an unreleased fix before the Web Store listing updates.',
  },
  {
    q: 'Which AI provider should I use?',
    a: 'We recommend xAI Grok 4.1 Fast for the best balance of speed and accuracy. It has a 2M token context window and costs $0.20/$0.50 per million tokens. For a free hosted option, try Gemini 2.0 Flash (experimental). For maximum quality, use Claude Opus 4.1 or Grok 4.1. If you want broad model access, use OpenRouter. If you want a privacy-friendly local setup with no API key, use LM Studio.',
  },
  {
    q: 'Which browsers are supported?',
    a: 'FSB supports Chrome 88+ and all Chromium-based browsers including Microsoft Edge, Brave, Opera, and Vivaldi. Firefox support is planned for a future release.',
  },
  {
    q: 'Are my API keys secure?',
    a: 'Yes. API keys are encrypted using AES-GCM before being stored in Chrome\u2019s local storage. They are only decrypted in-memory when making API calls. Keys are never logged, exported, or transmitted to any server other than your chosen AI provider.',
  },
  {
    q: 'Is FSB safe to use?',
    a: 'FSB includes action validation and safety checks. It only performs actions on the current tab and only when explicitly instructed. All code is open source and auditable. That said, use caution with tasks involving sensitive accounts or financial transactions -- always review what FSB is doing.',
  },
  {
    q: 'How do I report a bug?',
    a: 'Open an issue on the GitHub Issues page at https://github.com/lakshmanturlapati/FSB/issues. Include the website URL, your task description, and any error messages from the FSB chat or browser console. Session logs from the Debug tab in Options can also help.',
  },
  {
    q: 'What is MCP and how do I use it?',
    a: 'MCP (Model Context Protocol) lets you control FSB from AI coding tools like Claude Code, Codex, Cursor, Antigravity, and OpenClaw. The MCP server runs as `npx -y fsb-mcp-server` and pairs with the running extension over a local WebSocket bridge. FSB exposes 50+ browser actions as MCP tools (click, type, navigate, read DOM, fill vault credentials, and more). For OpenClaw users specifically, the fastest path is the one-click install on ClawHub at https://clawhub.ai/lakshmanturlapati/full-selfbrowsing. The manual fallback is the FSB skill at `skills/fsb/`, which prints the stdio config block and runs the doctor diagnostic. The full rundown lives at https://full-selfbrowsing.com/agents.',
  },
  {
    q: 'How does FSB help Claude Code or Codex test apps?',
    a: 'Connect FSB as an MCP server, then let the coding agent open the local app, click through real flows, observe DOM state, capture console or task errors, and report failures back into the coding session. This closes the manual loop between code generation, browser testing, logging, and fixing.',
  },
  {
    q: 'Can I control FSB remotely?',
    a: 'Yes. The Remote Dashboard at full-selfbrowsing.com lets you pair with your FSB extension via QR code, then send tasks, view a live DOM preview of the browser, and monitor current execution state from any device with a web browser.',
  },
  {
    q: 'Does FSB learn from my usage?',
    a: 'FSB has a local memory system with three layers: semantic memory (site navigation patterns), episodic memory (past action sequences), and procedural memory (successful workflows). This data stays on your device and helps FSB navigate familiar sites faster and avoid repeating mistakes. Memory auto-consolidates after enough sessions.',
  },
  {
    q: 'How do I set up FSB\u2019s MCP server with my IDE?',
    a: 'Run `npx -y fsb-mcp-server install --list` to see which MCP hosts the installer detects on your machine, then run the installer for one (for example `npx -y fsb-mcp-server install --claude-desktop`). Cursor, Codex, Antigravity, and others have matching flags from --list. For OpenClaw, start with the ClawHub install at https://clawhub.ai/lakshmanturlapati/full-selfbrowsing. Use the FSB skill from `skills/fsb/` only as a manual fallback because the bare --openclaw installer flag is intentionally manual while OpenClaw\u2019s MCP config schema changes across builds. Verify the install with `npx -y fsb-mcp-server doctor`; six green layers means you\u2019re ready.',
  },
  {
    q: 'What can I do with FSB through MCP tools?',
    a: 'FSB exposes two modes through MCP. Manual mode is the default and gives the calling agent 50+ granular tools -- navigate, click, type, scroll, read page state, take screenshots, fill spreadsheets, fill vault credentials, and more. Autopilot mode (`run_task`) only fires when the user explicitly delegates the whole task -- phrases like "use FSB autopilot", "delegate this", or naming `run_task` directly. For everything else the calling agent stays in the loop on every action, which is exactly what coding agents want when they\u2019re testing an app in-browser and feeding observations back into the editor.',
  },
  {
    q: 'How do I install FSB on OpenClaw?',
    a: 'Install from ClawHub first: https://clawhub.ai/lakshmanturlapati/full-selfbrowsing. If you need the manual fallback, load the FSB skill from `skills/fsb/` in the FSB repo. It prints the canonical OpenClaw stdio block and runs the doctor diagnostic. The full setup walkthrough lives at https://full-selfbrowsing.com/agents.',
  },
  {
